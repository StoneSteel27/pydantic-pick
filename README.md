...
> [!WARNING]
> **Model Validators are Dropped:** Both `@model_validator` and `@model_serializer` are ignored during extraction. Because model validators check the entire class state (e.g., checking if `password == confirm_password`), copying them to a subset class where fields might be missing would cause fatal `AttributeError`s at runtime. 
